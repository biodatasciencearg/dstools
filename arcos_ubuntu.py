...
ftps_dicc =     {
    ...
    'CL':{'USER':"515005349",'password':"#2202McCH#@@$", 'server':'mc7dx8wzs3cz278f84lf0yrcsxw8.ftp.marketingcloudops.com','NOMBRE':'CHILE'},
    'CO':{'USER':"100010422_2",'password':"Mc@23ewr@#r##", 'server':'mc7dx8wzs3cz278f84lf0yrcsxw8.ftp.marketingcloudops.com','NOMBRE':'COLOMBIA'},
    'CR':{'USER':"515005350",'password':"#@#MCfsa!#21@#13", 'server':'mc7dx8wzs3cz278f84lf0yrcsxw8.ftp.marketingcloudops.com','NOMBRE':'COSTA RICA'},
    ...
    'PA':{'USER':"515007932",'password':"wuV4!Si3P+7q8xW*", 'server':'mc7dx8wzs3cz278f84lf0yrcsxw8.ftp.marketingcloudops.com','NOMBRE':'PANAMA'},
    ...
    'UY':{'USER':"515005351",'password':"!@#!dw1d112mC!@", 'server':'mc7dx8wzs3cz278f84lf0yrcsxw8.ftp.marketingcloudops.com','NOMBRE':'URUGUAY'},
...
# Variable
gigigo_path = "/home/anto/OneDrive"
...
class db(object):
    def __init__(self):
        import psycopg2
        self.conn = psycopg2.connect(host="localhost",database="gigigo", user="postgres", password="asdasd")
...
def upload_file_salesforce(filename,country,verbose=True,progressEveryPercent=25):
    ...
    password =  str(ftps_dicc[country]['password'])
    logfile  = r"/home/anto/arcos_dorados/gigigo/automation/temp/log_loading_files.txt"
    cnopts = pysftp.CnOpts()
    cnopts.hostkeys = None
    with pysftp.Connection(host=host,username=username,password=password,log=logfile,cnopts=cnopts) as srv:
...
def update_all_countrys():
    ...
    logf = r"/home/anto/arcos_dorados/gigigo/automation/temp/logs_update_tables.txt"
...
def update_all_countrys_atributes():
    ...
    from contextlib import redirect_stdout
    # Defino el archivo donde pongo la salida.
    logf = r"/home/anto/arcos_dorados/gigigo/automation/temp/logs_atributes.txt"
...
def export_countries_DE(pais='all'):
    """Export todos los DE por pais en /tmp/ 
    Argumentos:
    ...
    # Defino el archivo donde pongo la salida.
    logf = r"/home/anto/arcos_dorados/gigigo/automation/temp/logs_exports.txt"
    # path de exportacion.
    export_path ='/tmp/'
    if pais=='all':
        # Defino la lista sobre las que itero.
    ...
    for country in paises:
        ...
        with open(logf, 'a+') as f:
             with redirect_stdout(f):
                    ...
                    print(date)
                    query = f"copy (select u.mcid, lower(unaccent(u.email)) as email, u.userfacebookid, u.country, u.firstname, u.lastname, u.gender, try_cast_date(u.birthdate) as birthdate, u.documento, u.createdat, u.smsenabled, u.pushenabled, u.emailenabled, u.showcouponalert, u.currentcity, u.phonenumberprefix, case when length(u.phonenumbersufix) >5 then u.phonenumbersufix else NULL end phonenumbersufix, u.status, u.rest_fav_1, u.rest_fav_2, u.rest_fav_3, r.last_burn, r.recency, r.frequency, days_since_creation, last_product, favorite_product, BTRIM(favorite_dayofweek, ' '), favorite_daypart, r.rfm_segment, r.control     from users u    left join rfm_history r on r.mcid=u.mcid and r.date=date((select max(date)::varchar  from rfm_history r))  where u.country='{country}' ) TO '{filename}' DELIMITER ';' CSV HEADER;"
                    run_sp(query)
                    now = datetime.datetime.now()
...
def import_countries_DE(pais='all'):
    ...
    yyyymmdd = time.strftime('%Y%m%d')
    # Defino el archivo donde pongo la salida.
    logf = r"/home/anto/arcos_dorados/gigigo/automation/temp/logs_import.txt"
    # path de exportacion.
    export_path ='/tmp/'
    if pais=='all':
        # Defino la lista sobre las que itero.
...
def ngrok_data():
    import subprocess
    import json
    proc = subprocess.Popen(["curl", "http://127.0.0.1:4040/api/tunnels"], stdout=subprocess.PIPE)
